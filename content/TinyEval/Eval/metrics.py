...
def classification_score(prediction, ground_truth, **kwargs):
    ...
    for match_term in em_match_list:
        if (
            match_term in ground_truth and match_term != ground_truth
...
def f1_score(prediction, ground_truth, **kwargs):
    ...
    if num_same == 0:
        return 0
    precision = (
        1.0 * num_same / len(prediction)
    )  # 即模型预测正确的样本数量与总预测样本数量的比值
    recall = (
        1.0 * num_same / len(ground_truth)
    ...
    if len(ground_truth) > 1:
        ...
        if matches:
            ...
            if predicted_answer == ground_truth:
                score = 1
            elif all(option in ground_truth for option in predicted_answer) and len(
                predicted_answer
            ) < len(ground_truth):
                score = 0.5
    else:
